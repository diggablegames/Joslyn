...
public class TextButton : Button, ICanvasRaycastFilter
{
		...
		/*private bool CheckIfPointInPolygon (Vector2[] polygon, Vector2 point)
	{
		...
		private void InternalDoStateTransition (int id, SelectionState state, bool instant)
		{
				...
				if (base.gameObject.activeInHierarchy) {
						...
						case Selectable.Transition.ColorTint:
								targetText.linksList [name] [id].CrossFadeColor (color * this.colors.colorMultiplier, (!instant) ? this.colors.fadeDuration : 0f, true, true, true);
								break;
		...
		public bool IsRaycastLocationValid (Vector2 sp, Camera eventCamera)
		{
				...
				lp.y -= targetText.m_CurveData.Evaluate ((targetText.rectT.rect.width * targetText.rectT.pivot.x + lp.x) / targetText.rectT.rect.width + targetText.phase) * targetText.curveMultiplier;
