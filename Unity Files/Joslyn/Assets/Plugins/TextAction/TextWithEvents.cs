...
public class TextWithEvents : Text
{
		...
		public float phase = 0; //for animation support
	...
	public void Restart(string newString){
		//OnValidate();
		text = newString;
		//nonParsedStr = newString;
		OnEnable();
	}
		...
		void changeRectData(){
				...
				OnBeforeValueChange (nonParsedStr);
				rectT.sizeDelta = new Vector2(rectT.rect.width, gameObject.GetComponent<Text>().preferredHeight);
		}
...
//						uiVertex = vbo [index];
//						uiVertex.position.y += m_CurveData.Evaluate ((rectT.rect.width * rectT.pivot.x + uiVertex.position.x) / rectT.rect.width + phase) * curveMultiplier;
//						vbo [index] = uiVertex;
//				}
//		}
}
