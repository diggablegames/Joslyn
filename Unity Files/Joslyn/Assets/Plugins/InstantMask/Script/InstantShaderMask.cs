...
[ExecuteInEditMode()]
public class InstantShaderMask : MonoBehaviour
{
	...
	bool BaseSpriteRepeatWarningCheck;
	bool BaseSpriteClampWarningCheck;
	...
	void Start ()
	{
		...
		if (Application.isEditor && !EditorApplication.isPlaying && !EditorApplication.isPaused) {
			...
			if (GetComponent<Image> ())
				BaseSpriteCheck = true;
			else
				BaseSpriteCheck = false;
			if (BaseSpriteCheck != true && BaseImage != null) {
			...
			} else if (BaseSpriteCheck && BaseSprite != null) {
				if (BaseSprite.texture.wrapMode == TextureWrapMode.Repeat) {
					BaseImageWrapMode = 0;
					HoldBaseImageWrapMode = 0;
				} else if (BaseSprite.texture.wrapMode == TextureWrapMode.Clamp) {
					BaseImageWrapMode = 1;
					HoldBaseImageWrapMode = 1;
				} else {
					BaseImageWrapMode = -1;
	...
	public void OnEnable ()
	{
		...
		BaseSpriteRepeatWarningCheck = false;
		BaseSpriteClampWarningCheck = false;
	...
	void Update ()
	{
		...
		if (GetComponent<Renderer> () && !GetComponent<SpriteRenderer> ()) {
			#if (UNITY_EDITOR)
			if (!EditorApplication.isPlaying && !EditorApplication.isPaused || !EditorApplication.isPlayingOrWillChangePlaymode) {
				if (RndMaterialsAry != GetComponent<Renderer> ().sharedMaterials)
					RndMaterialsAry = GetComponent<Renderer> ().sharedMaterials;
			} else {
				#endif
				if (RndMaterialsAry != GetComponent<Renderer> ().materials)
					RndMaterialsAry = GetComponent<Renderer> ().materials;
				#if (UNITY_EDITOR)
			}
			#endif
		...
		if (MaterialNum < 0)
			MaterialNum = 0;
		else if (MaterialNum > RndMaterialsAry.Length - 1)
			MaterialNum = RndMaterialsAry.Length - 1;
	
		if (GetComponent<Renderer> ()) {
			if (GetComponent<SpriteRenderer> ()) {
				BaseSpriteCheck = true;
				if (BaseSprite != HoldBaseSprite) {
					HoldBaseSprite = BaseSprite;
					GetComponent<SpriteRenderer> ().sprite = BaseSprite;
				}
				#if (UNITY_EDITOR)
				if (!EditorApplication.isPlaying && !EditorApplication.isPaused) {
					if (RndMaterial != GetComponent<SpriteRenderer> ().sharedMaterial)
						RndMaterial = GetComponent<SpriteRenderer> ().sharedMaterial;
				} else {
					#endif
					if (RndMaterial != GetComponent<SpriteRenderer> ().material)
						RndMaterial = GetComponent<SpriteRenderer> ().material;
					#if (UNITY_EDITOR)
				}
				#endif
			} else { //MeshRenderer & SkinnedMeshRenderer
				BaseSpriteCheck = false;
			...
			if (GetComponent<RawImage> ()) {
				BaseSpriteCheck = false;
				if (RndMaterial != GetComponent<RawImage> ().material)
					RndMaterial = GetComponent<RawImage> ().material;
				...
				if (RndMaterial != GetComponent<Image> ().material)
					RndMaterial = GetComponent<Image> ().material;
			} else {
				BaseSpriteCheck = false;
				RndMaterial = null;
			}
		...
		if (RndMaterial != null) {
			if (BaseImage == null && BaseSpriteCheck != true) {
				...
				UseMask2 = false;
				UseMask3 = false;
			} else if (BaseSprite == null && BaseSpriteCheck) {
				if(MaskAllImpossibleCheck != true)
				{
					if (UseMask1)
						Debug.LogWarning ("[InstantMask] Checked UseMask1, but BaseSprite is Not Found. So UseMask1 turn Off. GameObjegt name: " + gameObject.name);
					if (UseMask2)
						Debug.LogWarning ("[InstantMask] Checked UseMask2, but BaseSprite is Not Found. So UseMask2 turn Off. GameObjegt name: " + gameObject.name);
					if (UseMask3)
						Debug.LogWarning ("[InstantMask] Checked UseMask3, but BaseSprite is Not Found. So UseMask3 turn Off. GameObjegt name: " + gameObject.name);
				}
				MaskAllImpossibleCheck = true;
				UseMask1 = false;
				UseMask2 = false;
			...
			if (BaseImageWrapMode != HoldBaseImageWrapMode) {
				if (BaseImageWrapMode == 0) {
					if (BaseSpriteCheck != true && BaseImage != null)
						...
						BaseSprite.texture.wrapMode = TextureWrapMode.Repeat;
				} else {
					if (BaseSpriteCheck != true && BaseImage != null)
						BaseImage.wrapMode = TextureWrapMode.Clamp;
					else if (BaseSpriteCheck && BaseSprite != null)
						BaseSprite.texture.wrapMode = TextureWrapMode.Clamp;
				}
			...
			if (EditorApplication.isPlaying || EditorApplication.isPaused) {
				if (BaseSpriteCheck != true && BaseImage != null) {
					...
					if (BaseSprite.texture.wrapMode == TextureWrapMode.Repeat && BaseImageWrapMode != 0) {
						if (BaseSpriteRepeatWarningCheck != true && BaseImageWrapMode != -1)
							Debug.LogWarning ("[InstantMask] WrapMode of BaseSprite has been changed, current that is 'Repeat'. GameObjegt name: " + gameObject.name);
						BaseImageWrapMode = 0;
						HoldBaseImageWrapMode = 0;
						BaseImageRepeatWarningCheck = false;
						BaseImageClampWarningCheck = false;
						BaseSpriteRepeatWarningCheck = true;
						BaseSpriteClampWarningCheck = false;
					} else if (BaseSprite.texture.wrapMode == TextureWrapMode.Clamp && BaseImageWrapMode != 1) {
						if (BaseSpriteClampWarningCheck != true && BaseImageWrapMode != -1)
							Debug.LogWarning ("[InstantMask] WrapMode of BaseSprite has been changed, current that is 'Clamp'. GameObjegt name: " + gameObject.name);
						BaseImageWrapMode = 1;
						HoldBaseImageWrapMode = 1;
						BaseImageRepeatWarningCheck = false;
						BaseImageClampWarningCheck = false;
						BaseSpriteRepeatWarningCheck = false;
						BaseSpriteClampWarningCheck = true;
					}
				}
				if (Mask1Image != null) {
			...
			//メイン値とshader値のずれをチェック(shaderがすでにあってスクリプトをセットした直後用)
			if (GetComponent<RawImage> ()) {
				if (BaseImage != GetComponent<RawImage> ().texture as Texture) {
					BaseImage = GetComponent<RawImage> ().texture as Texture;
					HoldBaseImage = BaseImage;
				}
			}
			if (GetComponent<SpriteRenderer> ()) {
				if (BaseSprite != GetComponent<SpriteRenderer> ().sprite) {
					BaseSprite = GetComponent<SpriteRenderer> ().sprite;
					HoldBaseSprite = BaseSprite;
				}
			}
			if (GetComponent<Image> ()) {
				if (BaseSprite != GetComponent<Image> ().sprite) {
					BaseSprite = GetComponent<Image> ().sprite;
					HoldBaseSprite = BaseSprite;
				}
			}
			//他と共有のマテリアルを付けた際に既にスクリプトにセットされている画像が変わるのは嫌なので画像系だけはスクリプト側から統一
			...
			if(GetComponent<Image> () || GetComponent<RawImage> ()) {
				//UGUIのImage/RawImageコンポーネントはおそらくマテリアルのInspectorがない影響でPlay停止時にshaderの各値がスクリプトの値に合わないので
				//Undo時と同じ処理を走らせて値を強制で合わせる
				UndoRedoSet ();
			} else {
	...
	public Texture BaseImage {
		...
		set {
			if (value != HoldBaseImage) {
				HoldBaseImage = value;
				if (GetComponent<RawImage> ())
					GetComponent<RawImage> ().texture = BaseImage;
	...
	[HideInInspector, SerializeField] Sprite HoldBaseSprite;
	public Sprite BaseSprite {
		get { return HoldBaseSprite; }
		set {
			if (value != HoldBaseSprite) {
				HoldBaseSprite = value;
				if (GetComponent<SpriteRenderer> ())
					GetComponent<SpriteRenderer> ().sprite = BaseSprite;
				if (GetComponent<Image> ())
					GetComponent<Image> ().sprite = BaseSprite;
